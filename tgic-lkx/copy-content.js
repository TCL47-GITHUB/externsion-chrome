...
if (
  window.location.href.match(
    /^https:\/\/www\.thegioiic\.com\/console-admin\/products\/\d+\/edit$/
  )
) {
...
if (
  window.location.href.match(
    /^https:\/\/www\.thegioiic\.com\/[\w-]+$/
  )
) {
  // Danh sách các thẻ cần sao chép (bao gồm cả URL và thẻ <a> cuối cùng)
  const elements = [
    document.querySelector(".product-title-show h1"),
    document.querySelector(".crumb li:last-child a"), // Thẻ <a> cuối cùng
    window.location.origin + ";https://linhkienx.com", // URL của trang web
    // document.getElementById('slug'),
  ];
...
if (window.location.href.startsWith("https://admin.linhkienx.com/products/")) {
  // Danh sách các thẻ cần sao chép (bao gồm cả URL và thẻ <a> cuối cùng)
  const elements = [
